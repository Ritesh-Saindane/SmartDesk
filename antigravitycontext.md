...
## Caveats & Team Sharing Protocol
Because the Google Cloud project is currently in "Testing" mode and relies on specific credentials, sharing this project with the two other developers requires careful handling:

1. **Authentication Restrictions**: 
   - For other developers to authenticate and test the Google Calendar integration with their *own* Google accounts, **you must whitelist their email addresses** in the Google Cloud Console under the "OAuth consent screen > Test users" section. 
   - If they are not whitelisted, Google will throw an `Access blocked: Error 403` when they try to log in.
2. **Sharing Secrets via Archive**:
   - The repository is private and securely ignores secrets via `.gitignore`. 
   - To allow your teammates to test the Productivity Agent's functionality using the shared `smartdeskgenai@gmail.com` account, you will need to send them a secure archive (e.g., `.7z`) containing your `.env`, `credentials.json`, and `token.json` files. 
   - *Note: By sharing `token.json`, your teammates will bypass the browser login step entirely, as the app will use the pre-authorized token generated by your trusted device.*
